...
function watch() {
  gulp.watch("src/style/**/*.scss", gulp.parallel(styles)).on("change", () => {
    return reload();
  });
  gulp.watch("src/pug/**/*.pug", gulp.parallel("pug")).on("change", () => {
    return reload();
  });
  gulp.watch("src/js/**/*.js", gulp.series(js)).on("change", () => {
    return reload();
  });
...
function styles() {
  return gulp
    .src([path.src.main, path.src.dark, path.src.light])
    .pipe(sourcemaps.init())
    .pipe(sass())
    .pipe(prefixer())
    .pipe(clean_css())
...
gulp.task(
  "deploy",
  gulp.series("cleanBuild", styles, js, "bundle-sw", "pug", "cleanDeploy")
);

gulp.task(
  "default",
  gulp.series("cleanBuild", "build", "bundle-sw", gulp.parallel("sync", watch))
);
